...
app.get("/api/getUnclaimed", (req, res) => {
    const sql = `SELECT i.Item_ID, c.Category_Name, i.Item_Name, i.Item_Value, i.Item_Desc, ish.Status_FK, ish.ISH_Location, ish.ISH_Date, ish.ISH_Time, o.Officer_Badge, o.Officer_Lname 
                 FROM (Select h.Item_FK, h.ISH_Date, h.Status_FK, h.Officer_FK, h.ISH_Location, h.ISH_Time From Item_Status_History h, 
                 (Select Item_FK, MAX(ISH_Date) as mdate from Item_Status_History Group by Item_FK) t where t.Item_FK = h.Item_FK and t.mdate = h.ISH_Date) ish 
                 Join Item i on i.Item_ID = ish.Item_FK Left Join Officer o on o.Officer_Badge = ish.Officer_FK join Category c on i.Category_FK = c.Category_Name Where ish.Status_FK = 'Unclaimed' Order by ish.ISH_Date DESC`;
...
app.get("/api/getClaimed", (req, res) => {
    const sql = `SELECT i.Item_ID, c.Category_Name, i.Item_Name, i.Item_Value, i.Item_Desc, 
                        ...
                        o.Officer_Badge, o.Officer_Lname 
